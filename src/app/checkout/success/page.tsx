...
function CheckoutSuccessContent() {
	...
	const creditsUsed = searchParams.get("credits") === "true";
	...
	useEffect(() => {
		// If credits were used, skip order verification and show success immediately
		if (creditsUsed) {
			setOrder({
				reference: "CREDITS",
				status: "PAID",
		...
		const pollOrder = async () => {
			...
			try {
				...
				// Continue polling if pending and under max polls
				pollCount++;
		...
		const interval = setInterval(async () => {
			const shouldContinue = await pollOrder();
			if (!shouldContinue) {
	...
	}, [reference, creditsUsed, basketCleared, clearBasket]);
				...
				<div className="container mx-auto px-6 sm:px-8 lg:px-12 max-w-2xl text-center">
					...
					{reference && (
						<div className="bg-[#1a1a1a] rounded-lg p-6 mb-8">
							...
							<p className="font-mono text-yellow-500 text-xl tracking-wider">
								{reference}
							</p>
						</div>
					)}
		...
		<div className="py-24">
			<div className="container mx-auto px-6 sm:px-8 lg:px-12 max-w-2xl text-center">
				...
				<p className="text-gray-400 text-lg mb-8">
					Thank you for your booking, {order?.name}! A confirmation email will
				...
				{reference && (
					<div className="bg-[#1a1a1a] rounded-lg p-6 mb-8">
						<p className="text-gray-500 text-sm mb-2">Booking Reference</p>
						<p className="font-mono text-lime text-xl tracking-wider">
							{reference}
						</p>
					</div>
				)}
