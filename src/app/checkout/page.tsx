...
export default function CheckoutPage() {
	...
	useEffect(() => {
		...
		fetchCredits()
			.then(() => {
				// If we have credits or lastFetched is set (even if 0), user attempted to fetch credits
				// which means they're either logged in or the API returned 401
				const credits = useCreditsStore.getState();
				// User is logged in if they have credits OR if lastFetched is set (meaning API call completed)
				// If not logged in, fetchCredits will set lastFetched but totalCredits will be 0
				// We can't distinguish between "logged in with 0 credits" and "not logged in"
				// So we'll check by trying to use credits - if 401, redirect to login
				setIsLoggedIn(credits.lastFetched !== null);
			})
			.catch(() => {
				setIsLoggedIn(false);
			});
	...
	const handleUseCredits = async () => {
		...
		try {
			...
			if (totalCredits < items.length) {
				setError(
					`You need ${items.length} credits but only have ${totalCredits}. Please purchase more credits or use card payment.`,
				);
				setLoading(false);
				return;
			}

			// Check if user is logged in (if credits fetch failed with 401, they're not logged in)
			if (!isLoggedIn && totalCredits === 0) {
			...
			if (!response.ok) {
				...
				const errorMsg = data.error || "Failed to book with credits";
				const availableMsg =
					data.available !== undefined ?
						` (You have ${data.available} credits)`
					:	"";
				const requiredMsg =
					data.required !== undefined ? ` (Need ${data.required})` : "";
				throw new Error(`${errorMsg}${availableMsg}${requiredMsg}`);
			}
										...
										<div className="flex items-center justify-between">
											<div>
												<p className="text-white font-medium">
													Use Credits ({creditsToUse} credit
													{creditsToUse !== 1 ? "s" : ""})
												</p>
												{remainingItems > 0 ?
													<p className="text-gray-400 text-sm">
														+ £{remainingTotal.toFixed(2)} for remaining{" "}
														{remainingItems} session
														{remainingItems !== 1 ? "s" : ""}
													</p>
												:	<p className="text-lime text-sm">
														Instant booking - no payment needed
													</p>
												}
											</div>
											<div
								...
								<div className="mt-6">
									{useCredits ?
										<button
											...
											disabled={loading || totalCredits < items.length}
											...
											{loading ?
												"Booking..."
											: totalCredits >= items.length ?
												`Book with ${creditsToUse} Credit${creditsToUse !== 1 ? "s" : ""}`
											:	`Need ${items.length - totalCredits} more credit${items.length - totalCredits !== 1 ? "s" : ""}`
											}
										</button>
									:	<button
							...
							<>
								...
								<form onSubmit={handleSubmit} className="space-y-6">
									...
									<p className="text-gray-600 text-xs text-center">
										You will be redirected to SumUp's secure payment page to
