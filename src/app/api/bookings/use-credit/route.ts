...
export const POST = withAuth(async (request, { user }) => {
  try {
    ...
    const alreadyBooked = new Set(
      existingBookings.map((b) => `${b.sessionId}-${b.date.toISOString().split('T')[0]}`)
    )
    ...
    if (sessionsToBook.length === 0) {
      return NextResponse.json(
        { error: 'All sessions are already booked' },
        { status: 400 }
      )
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const session of sessionsToBook) {
        ...
        while (
          purchaseIndex < creditPurchases.length &&
          creditPurchases[purchaseIndex].creditsRemaining <= creditsUsed
        ) {
        ...
        if (purchaseIndex >= creditPurchases.length) {
          throw new Error('Ran out of credits during booking')
        }

        const currentPurchase = creditPurchases[purchaseIndex]
