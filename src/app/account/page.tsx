...
export default async function AccountPage() {
      ...
      <div className="py-16">
        <div className="container mx-auto px-6 sm:px-8 lg:px-12 max-w-4xl text-center">
          ...
          <p className="text-gray-400 mb-8">
            Supabase authentication is not yet configured. Please set up your environment variables.
          </p>
          <Link href="/" className="btn-lime text-sm font-semibold tracking-widest uppercase">
            Back to Home
          </Link>
        </div>
      </div>
  ...
  const firstName = prismaUser?.firstName || user.user_metadata?.firstName || ''
  ...
  const creditPurchases = prismaUser
  ...
  const totalCredits = creditPurchases.reduce((sum, p) => sum + p.creditsRemaining, 0)
  ...
  const earliestExpiration = creditPurchases.find((p) => p.expiresAt <= thirtyDaysFromNow)?.expiresAt
          ...
          <div className="bg-[#1a1a1a] rounded-lg p-6">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <p className="text-gray-500 text-sm">Track measurements & activity</p>
              </Link>
              <Link
              ...
              >
                ...
                <p className="text-gray-500 text-sm">Log exercises & progress</p>
              </Link>
              <Link
                href="/timetable"
                className="block w-full text-left p-3 rounded-lg bg-[#0a0a0a] hover:bg-lime/10 border border-gray-800 hover:border-lime/30 transition-colors"
                        ...
                        {order.bookings.map((booking) => (
                          <div key={booking.id} className="flex justify-between text-sm">
                            ...
                            <span className="text-gray-500">
                              {formatDate(booking.date)} at {formatTime(booking.session.startTime)}
                            </span>
                          </div>
                        ))}
                      </div>
